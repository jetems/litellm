...
const CreateSearchTool: React.FC<CreateSearchToolProps> = ({
  ...
  const handleCreate = async (formValues: Record<string, any>) => {
    ...
    try {
      ...
      if (accessToken != null) {
        ...
        NotificationsManager.success(t("Search tool created successfully"));
        form.resetFields();
        setFormValues({});
      ...
      NotificationsManager.error(t("Error creating search tool: ") + error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTestConnection = async () => {
    try {
      ...
      NotificationsManager.error(t("Please fill in Search Provider and API Key before testing"));
    }
  };
    ...
    <Modal
      title={
        <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
          ...
          <h2 className="text-xl font-semibold text-gray-900">{t("Add New Search Tool")}</h2>
        </div>
      }
      open={isModalVisible}
      width={800}
          ...
          <div className="grid grid-cols-1 gap-6">
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("Search Tool Name")}
                  <Tooltip title={t("A unique name to identify this search tool configuration (e.g., 'perplexity-search', 'tavily-news-search').")}>
              ...
              rules={[
                { required: true, message: t("Please enter a search tool name") },
                {
                  pattern: /^[a-zA-Z0-9_-]+$/,
                  message: t("Name can only contain letters, numbers, hyphens, and underscores"),
                },
              ]}
            >
              <TextInput
                placeholder={t("e.g., perplexity-search, my-tavily-tool")}
            ...
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("Search Provider")}
                  <Tooltip title={t("Select the search provider you want to use. Each provider has different capabilities and pricing.")}>
                    <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                  </Tooltip>
                </span>
              }
              name="search_provider"
              rules={[{ required: true, message: t("Please select a search provider") }]}
            >
              <Select
            ...
            <Form.Item
              label={
                <span className="text-sm font-medium text-gray-700 flex items-center">
                  {t("API Key")}
                  <Tooltip title={t("The API key for authenticating with the search provider. This will be securely stored.")}>
                    <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                  </Tooltip>
                </span>
              }
              name="api_key"
              rules={[{ required: false, message: t("Please enter an API key") }]}
            >
              <TextInput
                type="password"
            ...
            <Form.Item
              label={<span className="text-sm font-medium text-gray-700">{t("Description (Optional)")}</span>}
              name="description"
            >
              <TextArea
                rows={3}
                placeholder={t("Brief description of this search tool's purpose")}
                className="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500"
              />
            </Form.Item>
          </div>
