...
export const SearchToolTester: React.FC<SearchToolTesterProps> = ({ searchToolName, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      message.warning(t("Please enter a search query"));
      return;
    }
    ...
    try {
      ...
      NotificationsManager.fromBackend(t("Failed to query search tool"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Card className="mt-6">
      <div className="mb-6">
        <TremorTitle>{t("Test Search Tool")}</TremorTitle>
      </div>
        ...
        <div className="flex-1">
          {!latestResults && !isLoading ? (
            <div className="h-full flex flex-col items-center justify-center p-8">
              ...
              <Text className="text-lg text-gray-600 font-medium">{t("Test your search tool")}</Text>
              <Text className="text-sm text-gray-500 mt-2">{t("Enter a query above to see search results")}</Text>
            </div>
          ) : (
            <div>
              {isLoading && (
                ...
                <>
                  ...
                  <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg" style={{ boxShadow: "0 1px 2px 0 rgba(0, 0, 0, 0.05)" }}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <Text className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{t("Search Query")}</Text>
                      ...
                      <div className="text-right ml-4">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <div className="text-sm font-semibold text-blue-600">
                            {latestResults.response?.results?.length || 0} {latestResults.response?.results?.length === 1 ? t('result') : t('results')}
                          </div>
                  ...
                  {latestResults.response && latestResults.response.results && latestResults.response.results.length > 0 ? (
                          ...
                          >
                            <div className="p-5">
                              ...
                              {result.snippet.length > 200 && (
                                ...
                                >
                                  {isResultExpanded ? t("show_less") : t("show_more")}
                                </Button>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="text-center py-12 bg-gray-50 border border-gray-200 rounded-lg">
                      ...
                      <Text className="text-gray-600 font-medium">{t("No results found")}</Text>
                      <Text className="text-sm text-gray-500 mt-1">{t("Try a different search query")}</Text>
                    </div>
                  )}
                </>
              )}
              ...
              {searchHistory.length > 1 && (
                <div className="mt-8 pt-6 border-t border-gray-200">
                  <div className="flex items-center justify-between mb-4">
                    <Text className="text-sm font-semibold text-gray-700">{t("Previous Searches")}</Text>
                    <Button
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500 mt-1.5 flex items-center gap-2">
                          <span className="font-medium text-blue-600">
                            {entry.response?.results?.length || 0} {entry.response?.results?.length === 1 ? t('result') : t('results')}
                          </span>
