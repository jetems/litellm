...
export function LoggingSettingsView({
  ...
  const getEventTypeLabel = (eventType: string) => {
    switch (eventType) {
      case "success":
        return t("Success Only");
      case "failure":
        return t("Failure Only");
      case "success_and_failure":
        return t("Success & Failure");
      default:
        return eventType;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Text className="font-semibold text-gray-900">{t("Logging Integrations")}</Text>
        ...
        {loggingConfigs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Text className="text-xs text-blue-600">
                        {Object.keys(config.callback_vars).length} {t("parameters configured")}
                      </Text>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <Text className="text-gray-500 text-sm">{t("No logging integrations configured")}</Text>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Text className="font-semibold text-gray-900">{t("Disabled Callbacks")}</Text>
        ...
        {disabledCallbacks.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Text className="text-xs text-red-600">{t("Disabled for this key")}</Text>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <Text className="text-gray-500 text-sm">{t("No callbacks disabled")}</Text>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <Text className="font-semibold text-gray-900">{t("Logging Settings")}</Text>
            <Text className="text-xs text-gray-500">
              {t("Active logging integrations and disabled callbacks for this key")}
            </Text>
          </div>
        </div>
        {content}
  ...
  return (
    <div className={`${className}`}>
      <Text className="font-medium text-gray-900 mb-3">{t("Logging Settings")}</Text>
      {content}
    </div>
  );
}
