      ...
      <Card>
        <div className="space-y-6">
          <div className="flex justify-between items-start">
            <div>
              <Title>{t("Summary by User Agent")}</Title>
              <Subtitle>{t("Performance metrics for different user agents")}</Subtitle>
            </div>

            {/* User Agent Filter */}
            <div className="w-96">
              <Text className="text-sm font-medium block mb-2">{t("Filter by User Agents")}</Text>
              <Select
                mode="multiple"
                placeholder={t("All User Agents")}
                value={selectedTags}
                ...
                return (
                  <Card key={index}>
                    ...
                    <div className="mt-4 space-y-3">
                      <div>
                        <Text className="text-sm text-gray-600">{t("Success Requests")}</Text>
                        <Metric className="text-lg">{formatAbbreviatedNumber(tag.successful_requests)}</Metric>
                      </div>
                      <div>
                        <Text className="text-sm text-gray-600">{t("Total Tokens")}</Text>
                        <Metric className="text-lg">{formatAbbreviatedNumber(tag.total_tokens)}</Metric>
                      </div>
                      <div>
                        <Text className="text-sm text-gray-600">{t("Total Cost")}</Text>
      ...
      <Card>
        <TabGroup>
          <TabList className="mb-6">
            <Tab>{t("DAU/WAU/MAU")}</Tab>
            <Tab>{t("Per User Usage (Last 30 Days)")}</Tab>
          </TabList>
          ...
          <TabPanels>
            ...
            <TabPanel>
              <div className="mb-6">
                <Title>{t("DAU, WAU & MAU per Agent")}</Title>
                <Subtitle>{t("Active users across different time periods")}</Subtitle>
              </div>
              ...
              <TabGroup>
                ...
                <TabPanels>
                  <TabPanel>
                    <div className="mb-4">
                      <Title className="text-lg">{t("Daily Active Users - Last 7 Days")}</Title>
                    </div>
                    {dauLoading ? (
                      <ChartLoader isDateChanging={false} />
                    ) : (
                  ...
                  <TabPanel>
                    <div className="mb-4">
                      <Title className="text-lg">{t("Weekly Active Users - Last 7 Weeks")}</Title>
                    </div>
                    {wauLoading ? (
                      <ChartLoader isDateChanging={false} />
                    ) : (
                  ...
                  <TabPanel>
                    <div className="mb-4">
                      <Title className="text-lg">{t("Monthly Active Users - Last 7 Months")}</Title>
                    </div>
                    {mauLoading ? (
                      <ChartLoader isDateChanging={false} />
                    ) : (
