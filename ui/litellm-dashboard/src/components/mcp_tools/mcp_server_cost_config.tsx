    ...
    <Card>
      <div className="space-y-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <Tooltip title={t("Configure costs for this MCP server's tool calls. Set a default rate and per-tool overrides.")}>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t("Default Cost per Query ($)")}
              <Tooltip title={t("Default cost charged for each tool call to this server.")}>
            ...
            <Text className="block mt-1 text-gray-500 text-sm">
              {t("Set a default cost for all tool calls to this server")}
            </Text>
          </div>
          ...
          {tools.length > 0 && (
            <div className="space-y-4">
              <label className="block text-sm font-medium text-gray-700">
                {t("Tool-Specific Costs ($)")}
                <Tooltip title={t("Override the default cost for specific tools. Leave blank to use the default rate.")}>
                  <InfoCircleOutlined className="ml-1 text-gray-400" />
                </Tooltip>
              </label>
              <Collapse
                items={[
                  {
                    ...
                    label: (
                      <div className="flex items-center">
                        ...
                        <span className="font-medium">{t("Available Tools")}</span>
                        <Badge
                          count={tools.length}
                          style={{
        ...
        {(value.default_cost_per_query ||
          (value.tool_name_to_cost_per_query && Object.keys(value.tool_name_to_cost_per_query).length > 0)) && (
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <Text className="text-blue-800 font-medium">{t("Cost Summary")}:</Text>
              <div className="mt-2 space-y-1">
                {value.default_cost_per_query && (
                  <Text className="text-blue-700">
                    • {t("Default cost")}: ${value.default_cost_per_query.toFixed(4)} {t("per query")}
                  </Text>
                )}
                {value.tool_name_to_cost_per_query &&
                    ...
                    ([toolName, cost]) =>
                      ...
                      cost !== undefined && (
                        <Text key={toolName} className="text-blue-700">
                          • {toolName}: ${cost.toFixed(4)} {t("per query")}
                        </Text>
                      ),
                  )}
              </div>
