...
export const mcpServerColumns = (
  ...
  isLoadingHealth: boolean | undefined,
  t: (key: string) => string
): ColumnDef<MCPServer>[] => [
    {
      ...
      cell: ({ row }) => {
        ...
        const tooltipContent = (
          <div className="max-w-xs">
            <div className="font-semibold mb-1">{t("Health Status")}: {status}</div>
            {lastCheck && <div className="text-xs mb-1">{t("Last Check")}: {new Date(lastCheck).toLocaleString()}</div>}
            {error && (
            ...
            {!lastCheck && !error && <div className="text-xs text-gray-400">{t("No health check data available")}</div>}
          </div>
        );
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <Tooltip title={t("Edit MCP Server")}>
            <Icon
              icon={PencilAltIcon}
              size="sm"
          ...
          <Tooltip title={t("Delete MCP Server")}>
            <Icon
              icon={TrashIcon}
              size="sm"
