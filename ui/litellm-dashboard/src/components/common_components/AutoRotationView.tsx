...
const AutoRotationView: React.FC<AutoRotationViewProps> = ({
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Text className="font-semibold text-gray-900">{t("Auto-Rotation")}</Text>
          <Badge color={autoRotate ? "green" : "gray"} size="xs">
            {autoRotate ? t("Enabled") : t("Disabled")}
          </Badge>
          {autoRotate && rotationInterval && (
            <>
              <Text className="text-gray-400">•</Text>
              <Text className="text-sm text-gray-600">{t("Every")} {rotationInterval}</Text>
            </>
          )}
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          {lastRotationAt && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              ...
              <div className="flex-1">
                <Text className="font-medium text-gray-700">{t("Last Rotation")}</Text>
          ...
          {(keyRotationAt || nextRotationAt) && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              <ClockIcon className="w-4 h-4 text-gray-500" />
              <div className="flex-1">
                <Text className="font-medium text-gray-700">{t("Next Scheduled Rotation")}</Text>
          ...
          {autoRotate && !lastRotationAt && !keyRotationAt && !nextRotationAt && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-100 rounded-md">
              <ClockIcon className="w-4 h-4 text-gray-500" />
              <Text className="text-gray-600">{t("No rotation history available")}</Text>
            </div>
          )}
        </div>
      )}
      ...
      {!autoRotate && !lastRotationAt && !keyRotationAt && !nextRotationAt && (
        <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-100 rounded-md">
          ...
          <Text className="text-gray-600">{t("Auto-rotation is not enabled for this key")}</Text>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <Text className="font-semibold text-gray-900">{t("Auto-Rotation")}</Text>
            <Text className="text-xs text-gray-500">{t("Automatic key rotation settings and status for this key")}</Text>
          </div>
        </div>
        {content}
      </div>
  ...
  return (
    <div className={`${className}`}>
      <Text className="font-medium text-gray-900 mb-3">{t("Auto-Rotation")}</Text>
      {content}
    </div>
  );
};
