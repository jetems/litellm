      ...
      render: (groups: string[]) => (
        <>
          {groups.length > 0 ? (
            ...
            <Text className="text-gray-400 italic">{t("No groups mapped")}</Text>
          )}
        </>
      ),
    },
  ...
  return (
    <Card>
      <div className="flex items-center gap-3">
        ...
        <Title level={3}>{t("Role Mappings")}</Title>
      </div>
      <div className="space-y-8">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Title level={5}>{t("Group Claim")}</Title>
            <div>
              <Text code>{roleMappings.group_claim}</Text>
            </div>
          </div>
          <div>
            <Title level={5}>{t("Default Role")}</Title>
            <div>
