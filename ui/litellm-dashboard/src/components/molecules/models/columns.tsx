...
export const columns = (
  ...
  t: (key: string) => string
    ...
    {
      header: () => <span className="text-sm font-semibold">{t("Model ID")}</span>,
    ...
    {
      header: () => <span className="text-sm font-semibold">{t("Model Information")}</span>,
      ...
      cell: ({ row }) => {
        ...
        const tooltipContent = (
          <div>
            <div>
              <strong>{t("Provider:")}</strong> {model.provider || "-"}
            </div>
            <div>
              <strong>{t("Public Model Name:")}</strong> {displayName}
            </div>
            <div>
              <strong>{t("LiteLLM Model Name:")}</strong> {model.litellm_model_name || "-"}
            </div>
          </div>
        );
    ...
    {
      header: () => <span className="text-sm font-semibold">{t("Credentials")}</span>,
      ...
      cell: ({ row }) => {
        ...
        return credentialName ? (
          <Tooltip title={`${t("Credential")}: ${credentialName}`}>
          ...
          <div className="flex items-center space-x-2 max-w-[180px]">
            ...
            <span className="text-xs text-gray-400">{t("No credentials")}</span>
          </div>
        );
      },
    },
    {
      header: () => <span className="text-sm font-semibold">{t("Created By")}</span>,
        ...
        return (
          <div className="flex flex-col min-w-0 max-w-[160px]">
            ...
            <div
              ...
              title={isConfigModel ? t("Defined in config") : createdBy || t("Unknown")}
            >
              {isConfigModel ? t("Defined in config") : createdBy || t("Unknown")}
            </div>
            {/* Created At - Secondary */}
            <div
              className="text-xs text-gray-500 truncate mt-0.5"
              title={isConfigModel ? t("Config file") : createdAt || t("Unknown date")}
            >
              {isConfigModel ? "-" : createdAt || t("Unknown date")}
            </div>
          </div>
        );
      },
    ...
    {
      header: () => <span className="text-sm font-semibold">{t("Updated At")}</span>,
    ...
    {
      header: () => <span className="text-sm font-semibold">{t("Team ID")}</span>,
    ...
    {
      header: () => <span className="text-sm font-semibold">{t("Model Access Group")}</span>,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        `}
          >
            {model.model_info.db_model ? t("DB Model") : t("Config Model")}
          </div>
        );
      },
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center justify-end gap-2 pr-4">
            {isConfigModel ? (
              <Tooltip title={t("Config model cannot be deleted on the dashboard. Please delete it from the config file.")}>
                <Icon icon={TrashIcon} size="sm" className="opacity-50 cursor-not-allowed" />
              </Tooltip>
            ) : (
              <Tooltip title={t("Delete model")}>
                <Icon
                  icon={TrashIcon}
                  size="sm"
                  onClick={() => {
