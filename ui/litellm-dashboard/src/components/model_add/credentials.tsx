...
const CredentialsPanel: React.FC<CredentialsPanelProps> = ({ uploadProps }) => {
  ...
  const handleUpdateCredential = async (values: any) => {
    ...
    NotificationsManager.success(t("Credential updated successfully"));
  ...
  const handleAddCredential = async (values: any) => {
    ...
    NotificationsManager.success(t("Credential added successfully"));
  ...
  const handleDeleteCredential = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("Credential deleted successfully"));
      await refetchCredentials();
    } catch (error) {
      NotificationsManager.error(t("Failed to delete credential"));
    } finally {
  ...
  return (
    <div className="w-full mx-auto flex-auto overflow-y-auto p-2">
      <Button onClick={() => setIsAddModalOpen(true)}>{t("Add Credential")}</Button>
      <div className="flex justify-between items-center mt-4 mb-4">
        <Text>{t("Configured credentials for different AI providers. Add and manage your API credentials.")}</Text>
      </div>
      ...
      <Card>
        <Table>
          <TableHead>
            <TableRow>
              <TableHeaderCell>{t("Credential Name")}</TableHeaderCell>
