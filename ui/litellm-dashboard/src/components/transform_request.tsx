...
${formattedBody}
  ...
  const handleTransform = async () => {
    ...
    try {
      ...
      try {
        ...
        NotificationsManager.fromBackend(t("Invalid JSON in request body"));
        setIsLoading(false);
        return;
      }
      ...
      if (!accessToken) {
        NotificationsManager.fromBackend(t("No access token found"));
        setIsLoading(false);
        return;
      }
      ...
      NotificationsManager.fromBackend(t("Failed to transform request"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="w-full m-2" style={{ overflow: "hidden" }}>
      ...
      <p className="text-sm text-gray-500">{t("See how LiteLLM transforms your request for the specified provider.")}</p>
      <div
        style={{
          display: "flex",
          gap: "16px",
      ...
      >
        ...
        >
          <div style={{ marginBottom: "24px" }}>
            <h2 style={{ fontSize: "24px", fontWeight: "bold", margin: "0 0 4px 0" }}>{t("Original Request")}</h2>
            <p style={{ color: "#666", margin: 0 }}>
              {t("The request you would send to LiteLLM /chat/completions endpoint.")}
            </p>
          </div>
        ...
        >
          <div style={{ marginBottom: "24px" }}>
            <h2 style={{ fontSize: "24px", fontWeight: "bold", margin: "0 0 4px 0" }}>{t("Transformed Request")}</h2>
            <p style={{ color: "#666", margin: 0 }}>{t("How LiteLLM transforms your request for the specified provider.")}</p>
            <br />
            <p style={{ color: "#666", margin: 0 }} className="text-xs">
              {t("Note: Sensitive headers are not shown.")}
            </p>
          </div>
