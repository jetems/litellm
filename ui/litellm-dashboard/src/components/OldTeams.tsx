...
const Teams: React.FC<TeamProps> = ({
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await fetchTeams(accessToken, userID, userRole, currentOrg, setTeams);
      NotificationsManager.success(t("Team deleted successfully"));
    } catch (error) {
      NotificationsManager.fromBackend(t("Error deleting the team: ") + error);
    } finally {
    ...
    try {
      ...
      if (accessToken != null) {
        ...
        if (existingTeamAliases.includes(newTeamAlias)) {
          throw new Error(t("Team alias {0} already exists, please pick another alias").replace("{0}", newTeamAlias));
        }
      ...
      NotificationsManager.fromBackend(t("Error creating the team: ") + error);
    }
  };
                              ...
                              teams
                                ...
                                .map((team: any) => (
                                  <TableRow key={team.team_id}>
                                    ...
                                    >
                                      {team.created_at ? new Date(team.created_at).toLocaleDateString() : t("N/A")}
                                    </TableCell>
                                    <TableCell
                                      style={{
                                        maxWidth: "4px",
                                                  ...
                                                  <div className="flex flex-wrap gap-1">
                                                    ...
                                                    {team.models.length > 3 && !expandedAccordions[team.team_id] && (
                                                      <Badge size={"xs"} color="gray" className="cursor-pointer">
                                                        <Text>
                                                          +{team.models.length - 3}{" "}
                                                          {team.models.length - 3 === 1 ? t("more model") : t("more models")}
                                                        </Text>
                                                      </Badge>
                                                    )}
                                                    {expandedAccordions[team.team_id] && (
                                      ...
                                      {userRole == "Admin" ? (
                                        <>
                                          <TableIconActionButton
                                            ...
                                            tooltipText={t("Edit team")}
                                          />
                                          <TableIconActionButton
                                            variant="Delete"
                                            onClick={() => handleDelete(team)}
                                            dataTestId="delete-team-button"
                                            tooltipText={t("Delete team")}
                                          />
                                        </>
                                      ) : null}
                                    </TableCell>
                        ...
                        <DeleteResourceModal
                          ...
                          alertMessage={
                            teamToDelete?.keys?.length === 0
                              ? undefined
                              : t("Warning: This team has {0} keys associated with it. Deleting the team will also delete all associated keys. This action is irreversible.").replace("{0}", String(teamToDelete?.keys?.length))
                          }
                          message={t("Are you sure you want to delete this team and all its keys? This action cannot be undone.")}
                          ...
                          resourceInformation={[
                            ...
                            { label: t("Keys"), value: teamToDelete?.keys?.length },
                ...
                <>
                      ...
                      <>
                        ...
                        >
                          <Select2
                            ...
                            placeholder={hasNoOrgs ? t("No organizations available") : t("Search or select an Organization")}
                            onChange={(value) => {
                  ...
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: t("Please select at least one model"),
                      },
                    ]}
                    name="models"
                  >
                    ...
                    <AccordionBody>
                      ...
                      >
                        <Switch
                          disabled={!premiumUser}
                          checkedChildren={
                            premiumUser ? "Yes" : t("Premium feature - Upgrade to disable global guardrails by team")
                          }
                          unCheckedChildren={
                            premiumUser ? "No" : t("Premium feature - Upgrade to disable global guardrails by team")
                          }
                        />
                      </Form.Item>
                      <Form.Item
