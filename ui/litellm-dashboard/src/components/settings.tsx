        ...
        return (
          ...
          >
            {paramType === "password" ? (
              <Input.Password
                size="large"
                placeholder={`${t("Enter your")} ${fieldLabel.toLowerCase()}`}
...
const Settings: React.FC<SettingsPageProps> = ({ accessToken, userRole, userID, premiumUser }) => {
  ...
  const handleCallbackSubmit = async (formValues: Record<string, any>, callbackName: string, isEdit: boolean) => {
    ...
    try {
      ...
      NotificationsManager.success(
        isEdit ? t("Callback updated successfully") : `${t("Callback")} ${callbackName} ${t("added successfully")}`,
      );
  ...
  const handleOk = () => {
    ...
    addForm.validateFields().then((values) => {
      ...
      if (values.callback === "langfuse" || values.callback === "langfuse_otel") {
        ...
        payload = {
          error: t("Invalid callback value"),
        };
      }
  ...
  const confirmDeleteCallback = async () => {
    ...
    try {
      ...
      NotificationsManager.success(`${t("Callback")} ${callbackToDelete.name} ${t("deleted successfully")}`);
