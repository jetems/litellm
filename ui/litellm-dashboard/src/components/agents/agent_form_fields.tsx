  ...
  return (
    <>
      ...
      <Collapse defaultActiveKey={['basic']} style={{ marginBottom: 16 }}>
        {/* Basic Information */}
        <Panel header={`${t(AGENT_FORM_CONFIG.basic.title)} ${t("(Required)")}`} key={AGENT_FORM_CONFIG.basic.key}>
          {AGENT_FORM_CONFIG.basic.fields.map((field) => (
            <Form.Item
              key={field.name}
              label={t(field.label)}
              name={field.name}
              rules={field.required ? [{ required: true, message: `${t("Please enter")} ${t(field.label).toLowerCase()}` }] : undefined}
        ...
        {/* Skills */}
        <Panel header={`${t(AGENT_FORM_CONFIG.skills.title)} ${t("(Required)")}`} key={AGENT_FORM_CONFIG.skills.key}>
              ...
              <>
                {fields.map((field) => (
                  <div key={field.key} style={{ marginBottom: 16, padding: 16, border: '1px solid #d9d9d9', borderRadius: 4 }}>
                    <Form.Item
                      {...field}
                      label={t(SKILL_FIELD_CONFIG.id.label)}
                      name={[field.name, 'id']}
                    ...
                    <Form.Item
                      {...field}
                      label={t(SKILL_FIELD_CONFIG.name.label)}
                    ...
                    <Form.Item
                      {...field}
                      label={t(SKILL_FIELD_CONFIG.description.label)}
                    ...
                    <Form.Item
                      {...field}
                      label={t(SKILL_FIELD_CONFIG.tags.label)}
                    ...
                    <Form.Item
                      {...field}
                      label={t(SKILL_FIELD_CONFIG.examples.label)}
        ...
        {/* Capabilities */}
        <Panel header={t(AGENT_FORM_CONFIG.capabilities.title)} key={AGENT_FORM_CONFIG.capabilities.key}>
        ...
        {/* Optional Settings */}
        <Panel header={t(AGENT_FORM_CONFIG.optional.title)} key={AGENT_FORM_CONFIG.optional.key}>
        ...
        {/* LiteLLM Parameters */}
        <Panel header={t(AGENT_FORM_CONFIG.litellm.title)} key={AGENT_FORM_CONFIG.litellm.key}>
