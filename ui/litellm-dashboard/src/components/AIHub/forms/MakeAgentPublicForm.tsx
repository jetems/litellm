...
const MakeAgentPublicForm: React.FC<MakeAgentPublicFormProps> = ({
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      if (selectedAgents.size === 0) {
        NotificationsManager.fromBackend(t("Please select at least one agent to make public"));
        return;
      }
      setCurrentStep(1);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      NotificationsManager.success(`${t("Successfully made")} ${agentIdsToMakePublic.length} ${t("agent(s) public!")}`);
      handleClose();
      onSuccess();
    } catch (error) {
      console.error("Error making agents public:", error);
      NotificationsManager.fromBackend(t("Failed to make agents public. Please try again."));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Title>{t("Select Agents to Make Public")}</Title>
          <div className="flex items-center space-x-2">
            ...
            >
              {t("Select All")} {agentHubData.length > 0 && `(${agentHubData.length})`}
            </Checkbox>
          </div>
        </div>

        <Text className="text-sm text-gray-600">
          {t("Select the agents you want to be visible on the public model hub. Users will still require a valid Virtual Key to use these agents.")}
        </Text>
          ...
          <div className="space-y-3">
            {agentHubData.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Text>{t("No agents available.")}</Text>
              </div>
            ) : (
                      ...
                      {agent.skills && agent.skills.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          {agent.skills.length > 3 && (
                            <Text className="text-xs text-gray-500">+{agent.skills.length - 3} {t("more")}</Text>
                          )}
                        </div>
                      )}
                    </div>
        ...
        {selectedAgents.size > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <Text className="text-sm text-blue-800">
              <strong>{selectedAgents.size}</strong> {t("agent")}{selectedAgents.size !== 1 ? "s" : ""} {t("selected")}
            </Text>
          </div>
        )}
      </div>
  ...
  const renderStep2Content = () => {
    return (
      <div className="space-y-4">
        <Title>{t("Confirm Making Agents Public")}</Title>

        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <Text className="text-sm text-yellow-800">
            <strong>{t("Warning:")}</strong> {t("Once you make these agents public, anyone who can go to the")} { }
            <code>/ui/model_hub_table</code> {t("will be able to know they exist on the proxy.")}
          </Text>
        </div>

        <div className="space-y-3">
          <Text className="font-medium">{t("Agents to be made public:")}</Text>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">
            {t("Total:")} <strong>{selectedAgents.size}</strong> {t("agent")}{selectedAgents.size !== 1 ? "s" : ""} {t("will be made public")}
          </Text>
        </div>
      </div>
    );
