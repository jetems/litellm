  ...
  const handleTestGuardrails = async (text: string) => {
    ...
    if (results.length > 0) {
      NotificationsManager.success(
        t("Guardrails applied successfully", { count: results.length })
      );
    }
    if (errors.length > 0) {
      NotificationsManager.fromBackend(
        t("Guardrails failed", { count: errors.length })
      );
    }
  };
        ...
        <div className="flex h-full">
          ...
          <div className="w-1/4 border-r border-gray-200 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-auto">
              {isLoading ? (
                ...
                <div className="p-4">
                  <Empty
                    description={
                      searchQuery ? t("No guardrails match your search") : t("No guardrails available")
                    }
                  />
                </div>
              ) : (
                <List
                  ...
                  renderItem={(guardrail) => (
                    <List.Item
                      ...
                      className={`cursor-pointer hover:bg-gray-50 transition-colors px-4 ${selectedGuardrails.has(guardrail.guardrail_name || "")
            ...
            <div className="p-3 border-t border-gray-200 bg-gray-50">
              <Text className="text-xs text-gray-600">
                {t("Selected count", { selected: selectedGuardrails.size, total: filteredGuardrails.length })}
              </Text>
            </div>
          </div>
          ...
          <div className="w-3/4 flex flex-col bg-white">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
              <Title className="text-xl font-semibold mb-0">{t("Guardrail Testing Playground")}</Title>
            </div>
