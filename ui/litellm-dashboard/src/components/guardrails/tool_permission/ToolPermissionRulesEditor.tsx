...
const ToolPermissionRulesEditor: React.FC<ToolPermissionRulesEditorProps> = ({
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        <div>
          ...
          <Text className="text-sm text-gray-500">
            {t("Provide regex patterns for tool names or types and optionally constrain payload fields.")}
          </Text>
        </div>
        {!disabled && (
          <Button
          ...
          {config.rules.map((rule, index) => (
            <Card key={rule.id || index} className="bg-gray-50">
              <div className="flex items-center justify-between mb-3">
                <Text className="font-semibold">{t("Rule")} {index + 1}</Text>
                <Button
                  icon={<DeleteOutlined />}
                  danger
                  type="text"
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                  <Text className="text-sm font-medium">{t("Tool Name")} ({t("optional")})</Text>
                  <Input
                    disabled={disabled}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 mt-4">
                <div>
                  <Text className="text-sm font-medium">{t("Tool Type")} ({t("optional")})</Text>
                  <Input
                    disabled={disabled}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <Text className="text-sm font-medium flex items-center gap-1">
            ...
            <Tooltip title={t("Block returns an error when a forbidden tool is invoked. Rewrite strips the tool call but lets the rest of the response continue.")}>
              <InfoCircleOutlined />
            </Tooltip>
          </Text>
          <Select
      ...
      <div className="mt-4">
        <Text className="text-sm font-medium">{t("Violation message")} ({t("optional")})</Text>
        <Input.TextArea
          disabled={disabled}
          rows={3}
