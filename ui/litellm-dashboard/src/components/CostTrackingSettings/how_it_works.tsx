...
const HowItWorks: React.FC = () => {
  ...
  return (
    <div className="space-y-4 pt-2">
      <div>
        <Text className="font-medium text-gray-900 text-sm mb-1">{t("Cost Calculation")}</Text>
        <Text className="text-xs text-gray-600">
          {t("Discounts are applied to provider costs:")}{" "}
          <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{t("final_cost = base_cost × (1 - discount%/100)")}</code>
        </Text>
      </div>
      <div>
      ...
      <div>
        <Text className="font-medium text-gray-900 text-sm mb-1">{t("Valid Range")}</Text>
        <Text className="text-xs text-gray-600">
      ...
      <div className="pt-4 border-t border-gray-200">
        <Text className="font-medium text-gray-900 text-sm mb-2">{t("Validating Discounts")}</Text>
  ...
  -d '{
    ...
    "messages": [{"role": "user", "content": "Hello"}]
        ...
        <Text className="text-xs text-gray-600 mt-3 mb-2">
          {t("Look for these headers in the response:")}
        </Text>
        <div className="space-y-1.5">
          <div className="flex items-start gap-3">
            ...
            <Text className="text-xs text-gray-600">{t("Final cost after discount")}</Text>
          </div>
          <div className="flex items-start gap-3">
            <code className="bg-gray-100 px-2 py-1 rounded text-xs font-mono text-gray-800 whitespace-nowrap">
              x-litellm-response-cost-original
            </code>
            <Text className="text-xs text-gray-600">{t("Original cost before discount")}</Text>
          </div>
          <div className="flex items-start gap-3">
            <code className="bg-gray-100 px-2 py-1 rounded text-xs font-mono text-gray-800 whitespace-nowrap">
              x-litellm-response-cost-discount-amount
            </code>
            <Text className="text-xs text-gray-600">{t("Amount discounted")}</Text>
          </div>
        </div>
      </div>

      <div className="pt-4 border-t border-gray-200">
        <Text className="font-medium text-gray-900 text-sm mb-3">{t("Discount Calculator")}</Text>
        <Text className="text-xs text-gray-600 mb-3">
        ...
        {calculatedDiscount && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <Text className="text-sm font-medium text-blue-900 mb-2">{t("Calculated Results")}</Text>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Text className="text-xs text-blue-800">{t("Original Cost")}: </Text>
                <code className="text-xs font-mono text-blue-900">${calculatedDiscount.originalCost}</code>
              </div>
              <div className="flex items-center justify-between">
                <Text className="text-xs text-blue-800">{t("Final Cost")}: </Text>
                <code className="text-xs font-mono text-blue-900">${calculatedDiscount.finalCost}</code>
              </div>
              <div className="flex items-center justify-between">
                <Text className="text-xs text-blue-800">{t("Discount Amount")}: </Text>
                <code className="text-xs font-mono text-blue-900">${calculatedDiscount.discountAmount}</code>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-blue-300">
                <Text className="text-xs font-semibold text-blue-900">{t("Discount Applied")}: </Text>
