...
const CostTrackingSettings: React.FC<CostTrackingSettingsProps> = ({
  ...
  const handleRemoveProvider = async (provider: string, providerDisplayName: string) => {
    modal.confirm({
      ...
      content: `${t("Are you sure you want to remove the discount for")} ${providerDisplayName}?`,
      okText: t('Remove'),
      okType: 'danger',
      cancelText: t('Cancel'),
  ...
  const handleRemoveMargin = async (provider: string, providerDisplayName: string) => {
    modal.confirm({
      title: t('Remove Provider Margin'),
      icon: <ExclamationCircleOutlined />,
      content: `${t("Are you sure you want to remove the margin for")} ${providerDisplayName}?`,
      okText: t('Remove'),
      okType: 'danger',
      cancelText: t('Cancel'),
  ...
  return (
    <div className="w-full p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
          ...
          <Text className="text-gray-500 mt-1">
            {t("Configure cost discounts and margins for different LLM providers. Changes are saved automatically.")}
          </Text>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow w-full max-w-full space-y-4">
          ...
          <Accordion>
            <AccordionHeader className="px-6 py-4">
              <div className="flex flex-col items-start w-full">
                <Text className="text-lg font-semibold text-gray-900">{t("Provider Discounts")}</Text>
                <Text className="text-sm text-gray-500 mt-1">
                  {t("Apply percentage-based discounts to reduce costs for specific providers")}
                </Text>
              </div>
            </AccordionHeader>
        ...
        {isProxyAdmin && (
          <Accordion>
            <AccordionHeader className="px-6 py-4">
              <div className="flex flex-col items-start w-full">
                <Text className="text-lg font-semibold text-gray-900">{t("Fee/Price Margin")}</Text>
                <Text className="text-sm text-gray-500 mt-1">
                  {t("Add fees or margins to LLM costs for internal billing and cost recovery")}
                </Text>
              </div>
            </AccordionHeader>
            <AccordionBody className="px-0">
              <div className="p-6">
                ...
                {isFetching ? (
                  ...
                  <div className="py-16 px-6 text-center">
                    ...
                    <Text className="text-gray-700 font-medium mb-2">
                      {t("No provider margins configured")}
                    </Text>
                    <Text className="text-gray-500 text-sm">
                      {t("Click \"Add Provider Margin\" to get started")}
                    </Text>
                  </div>
                )}
              </div>
        ...
        <Accordion defaultOpen={true}>
          <AccordionHeader className="px-6 py-4">
            <div className="flex flex-col items-start w-full">
              <Text className="text-lg font-semibold text-gray-900">{t("Pricing Calculator")}</Text>
              <Text className="text-sm text-gray-500 mt-1">
                {t("Estimate LLM costs based on expected token usage and request volume")}
              </Text>
            </div>
          </AccordionHeader>
          <AccordionBody className="px-0">
      ...
      <Modal
        title={
          <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
            <h2 className="text-xl font-semibold text-gray-900">{t("Add Provider Margin")}</h2>
          </div>
        }
      ...
      >
        <div className="mt-6">
          <Text className="text-sm text-gray-600 mb-6">
            {t("Select a provider (or \"Global\" for all providers) and configure the margin. You can use percentage-based or fixed amount.")}
          </Text>
          <Form
            form={marginForm}
            layout="vertical"
